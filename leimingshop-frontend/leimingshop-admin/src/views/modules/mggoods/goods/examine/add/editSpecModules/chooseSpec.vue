        ...
        <el-form-item label="商品规格：" >
            ...
            <div v-for="(item,index) in specLevel_1" style="margin-bottom:28px;">
                ...
                <span v-for="(item2,index2) in specLevel_2[index]" style="margin-left:20px;" class="itemSkuWrarp itemSku2Wrarp">
                    <el-checkbox :disabled="true" v-model="item2.checked"  @change.native="composeTableData(2,index,index2)"></el-checkbox>
                ...
                <el-input
                    ...
                    ></el-input>
                <el-button :disabled="true" type="primary" plain v-else @click="addSepcFn(index)" size="mini"  style="margin-left:16px;">+自定义</el-button >
            </div>
            <!-- 编辑商品不能添加规则组 -->
            <el-button :disabled="true" type="primary"  v-if="!dataForm.id" @click="addSepcGroupFn()">添加规格组</el-button>
      ...
      addInputConfirm(index){
        ...
        // 判断同组规格属性值是否重复
          var ret2 =  this.specLevel_2[index].filter(item => item.name == this.inputValue)
           ...
           // 判断 是否输入为空了
          if(!this.specLevel_2[index][this.specLevel_2[index].length-1].name.replace(/ /g,'')){
              this.$message({
                  message:"规格属性值不能为空!",
                  type: 'warning',
                  duration: 800,
