      ...
      <el-table v-loading="dataListLoading" :data="dataList" border style="width: 100%;">
        ...
        <el-table-column :label="$t('handle')" fixed="right" header-align="center" align="center" width="150">
          <template slot-scope="scope">
            ...
            <el-button v-if="$hasPermission('sys:dept:delete')" class="artdanger" type="text" size="small" @click="deleteHandle(scope.row.id)">{{ $t('delete') }}</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 弹窗, 新增 / 修改 -->
  ...
  data () {
    return {
      mixinViewModuleOptions: {
        getDataListURL: '/admin-api/dept/list',
        deleteURL: '/admin-api/dept'
      }
    }
  },
  components: {
