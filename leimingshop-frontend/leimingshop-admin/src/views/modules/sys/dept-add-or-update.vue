    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmitHandle()" label-width="120px">
      ...
      <el-form-item prop="parentName" :label="$t('dept.parentName') + '：'" class="dept-list">
        ...
        <el-input v-model="dataForm.parentName" v-popover:deptListPopover :readonly="true" :placeholder="$t('dept.parentName')">
          <i
            v-if="$store.state.user.superadmin-api === 1 && dataForm.pid !== '0'"
            slot="suffix"
...
export default {
  ...
  methods: {
    init () {
      ...
      this.$nextTick(() => {
        ...
        this.getDeptList().then(() => {
          if (this.dataForm.id) {
            this.getInfo()
          } else if (this.$store.state.user.superadmin-api === 1) {
    ...
    // 获取信息
    getInfo () {
      this.$http.get(`/admin-api/dept/${this.dataForm.id}`).then(({ data: res }) => {
        if (res.code !== 200) {
          return this.$message.error(res.msg)
        }
        this.dataForm = {
    ...
    dataFormSubmitHandle: debounce(function () {
      this.$refs['dataForm'].validate((valid) => {
        ...
        this.$http[!this.dataForm.id ? 'post' : 'put']('/admin-api/dept', this.dataForm).then(({ data: res }) => {
          if (res.code !== 200) {
            return this.$message.error(res.msg)
          }
          this.$message({
