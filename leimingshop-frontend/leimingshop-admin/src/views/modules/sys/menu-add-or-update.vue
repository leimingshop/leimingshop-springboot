...
export default {
  ...
  methods: {
    ...
    // 获取菜单列表
    getMenuList () {
      return this.$http.get('/admin-api/menu/list?type=0').then(({ data: res }) => {
        if (res.code !== 200) {
    ...
    // 获取信息
    getInfo () {
      this.$http.get(`/admin-api/menu/${this.dataForm.id}`).then(({ data: res }) => {
        if (res.code !== 200) {
          return this.$message.error(res.msg)
        }
        this.dataForm = {
    ...
    dataFormSubmitHandle: debounce(function () {
      this.$refs['dataForm'].validate((valid) => {
        ...
        this.$http[!this.dataForm.id ? 'post' : 'put']('/admin-api/menu', this.dataForm).then(({ data: res }) => {
          if (res.code !== 200) {
            return this.$message.error(res.msg)
          }
          this.$message({
