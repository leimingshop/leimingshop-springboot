  ...
  data () {
    return {
      mixinViewModuleOptions: {
        getDataListURL: '/admin-api/job/schedule/page',
        getDataListIsPage: true,
        deleteURL: '/admin-api/job/schedule',
        deleteIsBatch: true
      },
      dataForm: {
        beanName: ''
  ...
  methods: {
    ...
    pauseHandle (id) {
      ...
      this.$confirm(this.$t('prompt.info', { 'handle': this.$t('schedule.pause') }), this.$t('prompt.title'), {
        ...
        this.$http.put('/admin-api/job/schedule/pause', id ? [id] : this.dataListSelections.map(item => item.id)).then(({ data: res }) => {
          if (res.code !== 200) {
    ...
    resumeHandle (id) {
      ...
      this.$confirm(this.$t('prompt.info', { 'handle': this.$t('schedule.resume') }), this.$t('prompt.title'), {
        ...
        this.$http.put('/admin-api/job/schedule/resume', id ? [id] : this.dataListSelections.map(item => item.id)).then(({ data: res }) => {
          if (res.code !== 200) {
            return this.$message.error(res.msg)
          }
          this.$message({
    ...
    runHandle (id) {
      ...
      this.$confirm(this.$t('prompt.info', { 'handle': this.$t('schedule.run') }), this.$t('prompt.title'), {
        ...
        this.$http.put('/admin-api/job/schedule/run', id ? [id] : this.dataListSelections.map(item => item.id)).then(({ data: res }) => {
          if (res.code !== 200) {
            return this.$message.error(res.msg)
          }
          this.$message({
