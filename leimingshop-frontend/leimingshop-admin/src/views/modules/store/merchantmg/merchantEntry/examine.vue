	...
	<div>
        ...
        <div v-loading="backScanLoading" style="margin-bottom:40px;">
            <el-form :inline="false"  label-width="150px">
                <div style="display:flex;justify-content: space-around;margin-bottom:20px;font-weight:600;color:rgba(0,0,0,1)" >
                      <!-- 店铺信息 -->
                    <div class="formMode" >
                        <div style="margin-bottom:20px;font-weight:600;color:rgba(0,0,0,1);">店铺信息</div>
        ...
        color:rgba(0,0,0,1);" class="">经营范围</div>
                        <div class="businessScope" >
                            <!-- <div v-for="(item,index) in dataInfo.storeGoodsClassDTOS">
                                {{item.gcName}}
                            </div> -->
                            <el-tree
                ...
                <div class="secondForm">
                    ...
                    <div class="proofNews" style="margin-bottom:20px;font-weight: 650;width:400px;">
                        <div class="title weightedFont">资质证明</div>
            ...
            <div style="display: flex;justify-content: space-around;margin-top:20px">
                <div style="display: flex;justify-content: space-around;margin-bottom:20px;font-weight:600;color:rgba(0,0,0,1);" class="formMode">入驻审核</div>
                <div style="display: flex;justify-content: space-around;margin-top:20px;width: 700px" class="formMode" ></div>

            </div>
            <el-form :inline="false" :model="dataForm" :rules="dataRule" ref="dataForm"   label-width="200px" style="display: flex;justify-content: space-around;margin-bottom:20px;font-weight:600;color:rgba(0,0,0,1); ">
                <div >
                <el-form-item label="审核结果：" style="margin-bottom:20px;font-weight:600;color:rgba(0,0,0,1);" prop="registerAuditStatus">
            ...
            <div style="display: flex;justify-content: center;margin-top:20px" v-if="row.operationType=='examine'">
                    <el-button type="primary" size="medium" @click="dataFormSubmit('dataForm')" :disabled="saveLoading" :loading="saveLoading">{{saveLoading ? "审核中···" : "确定"}}</el-button>

                    <el-button @click="showListFn"  size="medium" >取消</el-button>
                </div>

        </div>

        <!--返回按钮-->
        <div style="margin-top: 40px;margin-bottom: 40px" v-if="row.operationType=='look'">
            <el-button type="primary"  size="medium" style="display:block;margin:0 auto" @click="showListFn()">返回</el-button>
        </div>
...
import {storeAuditInfo,shopAuditUpdateStatus,allGoodsclass} from  "@/api/api.js"
export default {
	data () {
            ...
            storeAuditInfo(obj).then((res)=>{
                ...
                if(res.code==200){
                    ...
                    if(this.row.operationType=="look"){
                         this.dataForm.registerAuditStatus = res.data.registerAuditStatus
                          this.dataForm.registerAuditCause = res.data.registerAuditCause
                    }
                    this.expandedKeys = [];
                    this.checkedKeys = [];
