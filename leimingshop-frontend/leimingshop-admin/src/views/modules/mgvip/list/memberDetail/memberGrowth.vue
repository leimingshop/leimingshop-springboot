<template>
  <div>
    <el-table
      ...
      <el-table-column prop="pointDesc" label="成长值来源" align="center"></el-table-column>
      <el-table-column prop="pointValue" label="成长值变化" align="center">
        <template slot-scope="scope">
          <span v-if="scope.row.pointValue >= 0">+{{scope.row.pointValue}}</span>
          <span v-if="scope.row.pointValue < 0">{{scope.row.pointValue}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="currentValue" label="剩余成长值" align="center"></el-table-column>
  ...
  data() {
    return {
      mixinViewModuleOptions: {
        getDataListURL: memberPointPage+'?pointType=2',
