<template>
    <div>
        <div id="control-area">
            ...
            >
                <template slot="title">
                    ...
                    <div class="iconSize">
                        1、未开始状态——点击商品跳转到添加商品列表，可添加或删除商品，可修改商品秒杀库存\进行中状态——点击商品跳转到添加商品列表，可删除商品，不可修改商品秒杀库存
                    </div>
                    <div class="iconSize">
                        2、未开始状态——添加活动商品时弹窗活动库存默认为空，为空项的sku不参加秒杀活动，添加的商品sku秒杀库存至少有一个不为空
                    </div>
                    <div class="iconSize">
                        3、锁定商品库存规则——添加秒杀活动时添加的商品设置好秒杀库存后即锁定库存
                    </div>
                    <div class="iconSize">
                        4、用户在秒杀活动时间范围内，选择秒杀库存下单后取消订单，该订单秒杀库存退还到普通库存。
                    </div>
                    <div class="iconSize">
                        5、秒杀活动商品在秒杀时间范围内商品不可进行编辑，编辑提示该商品为秒杀活动商品，请在秒杀活动移除该商品
                    </div>
                </template>
            </el-alert>
        </div>
        ...
        >
            ...
            <!-- <el-table-column prop="id" label="秒杀活动ID" align="center" width="180"></el-table-column> -->
            <el-table-column
                prop="activityName"
                label="秒杀活动名称"
                align="center"
                ...
                <template slot-scope="scope">
                    ...
                    <el-button
                        ...
                        v-if="
                            ...
                            $hasPermission('sys:seckill:edit')
                        "
                        @click="stopSeckillActivity(scope.row)"
                        >停止
                    </el-button>
                </template>
            </el-table-column>
...
<script>
    ...
    import { getSeckillActivityPage } from "@/api/url";
    import { stopSeckillActivity } from "@/api/api";
    ...
    export default {
        ...
        data() {
            return {
                breaddata: ["营销系统", "秒杀活动", "秒杀活动列表"],
                mixinViewModuleOptions: {
                    activatedIsNeed: false,
                    getDataListURL: getSeckillActivityPage,
                    getDataListIsPage: true,
                    deleteURL: "/seller-api/seckill/activity",
                    deleteIsBatch: true,
                },
                // 生效时间
                valuetime: [],
        ...
        methods: {
            ...
            // 停止秒杀活动
            stopSeckillActivity(row) {
                this.$confirm("您确认停止该秒杀活动吗?", "提示", {
                    cancelButtonText: "取消",
                    ...
                    stopSeckillActivity(obj).then((res) => {
                        if (res.code == 200) {
                            this.$message({
                                message: res.msg,
                                type: "success",
