<template>
    <div style="margin-left:-30px;">
       <div v-for="(item,index) in imgList" class="imgWarpList" >
            <div  class="specName1" style="">{{item.specName1}}:</div>
            ...
            <div class="samllPic" v-if="item.imgListItem[0].image">
                <draggable v-model="item.imgListItem" draggable=".item" class="dragga">
                  <div v-for="(element,index2) in item.imgListItem" v-if="index2!=0" :key="Math.ceil(Math.random()*1000000)" class="item" style="margin-right:10px;margin-bottom: 10px">
  ...
  data () {
    return {
        imgList:[
            ...
            //   imgListItem:[
            //     {
            //       image:'http://morefun.image.alimmdn.com/paimai/file/banner.jpg',
            //       specId:"规格属性id",//如颜色,尺码 的id
            //       id:"规格属性值id", //如红色，m码 的id
            //     },
            //     {
            ...
            //     },
            //     {
            //       image:'http://morefun.image.alimmdn.com/lancomeC2B/gift_4.png'
            //     },
            //   ],
            // },
        ],
  ...
  methods: {
       // 编辑点进来的图片回显
        editImgBackSacn(imgList){
            ...
            this.setSpecAttributePictureSaveDTOList();
        ...
        //         that.setSpecAttributePictureSaveDTOList();
        //         that.$message({
        //           message: "图片上传成功!",
        //           type: 'success',
        //           duration: 800,
        ...
        setSpecAttributePictureSaveDTOList(){
          let that = this;
          var specAttributePictureSaveDTOList = [
            // {
          ...
          that.imgList.forEach((item,index)=>{
             // 方式编辑回显时为空时数组
              ...
              item.imgListItem.forEach((item2,index2)=>{
                    ...
                    specAttributePictureSaveDTOList.push(obj);
              })

          })
          console.log(specAttributePictureSaveDTOList)
          this.$emit("setSpecAttributePictureSaveDTOList",specAttributePictureSaveDTOList);
            this.$emit("setSpecImgList",this.imgList);
        },
        addPeople(){
