  ...
  <el-card shadow="never" class="aui-card--fill">
    <div class="mod-sys__role">
      ...
      <el-form>
          ...
          <el-button v-if="$hasPermission('sys:role:delete')" type="danger" plain @click="deleteHandle()">{{ $t('deleteBatch') }}</el-button>
        </el-form-item>
      </el-form>
      <el-table
        ...
        <el-table-column :label="$t('handle')" fixed="right" header-align="center" align="center" width="150">
          <template slot-scope="scope">
            ...
            <el-button v-if="$hasPermission('sys:role:delete')" type="text" size="small" class="artdanger" @click="deleteHandle(scope.row.id)">{{ $t('delete') }}</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
