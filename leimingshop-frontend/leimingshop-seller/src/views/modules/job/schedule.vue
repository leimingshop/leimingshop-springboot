<template>
  <el-card shadow="never" class="aui-card--fill">
    <div class="mod-job__schedule">
      ...
      <el-form>
      	<el-form-item>
          <el-button v-if="$hasPermission('sys:schedule:delete')" type="danger" plain @click="deleteHandle()">{{'批量' + $t('deleteBatch') }}</el-button>
          <el-button v-if="$hasPermission('sys:schedule:pause')" type="primary" plain @click="pauseHandle()">{{ $t('schedule.pauseBatch') }}</el-button>
          <el-button v-if="$hasPermission('sys:schedule:resume')" type="primary" plain @click="resumeHandle()">{{ $t('schedule.resumeBatch') }}</el-button>
          <el-button v-if="$hasPermission('sys:schedule:run')" type="primary" plain @click="runHandle()">{{ $t('schedule.runBatch') }}</el-button>
      ...
      <el-table
        ...
        <el-table-column :label="$t('handle')" fixed="right" header-align="center" align="center" width="150">
          <template slot-scope="scope">
            ...
            <el-button v-if="$hasPermission('sys:schedule:delete')" type="text" size="small" class="artdanger" @click="deleteHandle(scope.row.id)">{{ $t('delete') }}</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
