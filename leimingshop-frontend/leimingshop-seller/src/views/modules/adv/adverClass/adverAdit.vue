<template>
  <div class="adverClass">
    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm"  @keyup.enter.native="submitStore()" label-width="130px">
        <el-form-item prop="relationId" label="广告分类：">
...
import { uploadPicBase64,firstClass,adverDetail } from '@/api/api'
  ...
  // mixins: [mixinViewModule],
    data () {
        return {
            ...
            breaddata: ["运营管理",  "分类广告列表",'分类广告编辑'],
            breaddatas: ["运营管理",  "分类广告列表",'分类广告新增'],
            buttonStatus:false,
            subStatus:true,
            uploading:false,
            advId:null,
            ...
            this.$refs['dataForm'].validate((valid) => {
                if (valid) {
                    ...
                    if(this.advId){//编辑
                        ...
                        this.slectList.map((item,index)=>{
                            if(item.id==this.dataForm.relationId){
                                this.dataForm.relationName=item.gcName
                            }
                        })
