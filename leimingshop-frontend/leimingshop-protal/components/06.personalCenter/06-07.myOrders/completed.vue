        ...
        <Card class="order-info">
                        ...
                        <p class="p-info">
                            ...
                            >
                                <em
                                    ...
                                    "
                                    >{{
                                        item.activityType | filterOrderLabel
                                    }}</em
                                >
            ...
            {{ data.orderAmount }}
                ...
                <div class="o-status" :style="{ width: thead[3].width }">
                    <!--  {{ status.title }}
          <Button
            :class="status[status.select].class"
            :type="status[status.select].type"
            v-text="status[status.select].title"
            @click="status[status.select].handlerWhy"
          ></Button>-->
                    <span>已完成</span>
                    <Button
                        v-show="data.evaluateStatus == 0"
                        @click="handleEvaluate"
                        type="text"
                        >评价商品</Button
                    >
                    <Button
                        v-show="data.evaluateStatus == 1"
                ...
                <div class="o-operate" :style="{ width: thead[4].width }">
                    ...
                    <Button
                        type="text"
                        v-if="
                            data.orderGoodsDTOList.length === 1 &&
                            data.orderGoodsDTOList[0].aftersaleQuantity === 0
                        "
                        class="after-saled"
                        >已申请售后</Button
                    >
                    <Button type="text" v-else @click="handleAfterSale"
                        >申请售后</Button
                    >
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                ...
                status: {
                    ...
                    function1: {
                        title: "评价商品",
                        handlerWhy: this.handleEvaluate,
                        type: "text",
                    },
                    function2: {
                        title: "查看评价",
                        handlerWhy: this.handleEvaluateDetail,
                        type: "text",
                        class: "evaluateDetail",
                    },
                    select: "function2",
        ...
        methods: {
            ...
            handleAfterSale() {
                let tempList = this.data.orderGoodsDTOList;
                // 跳转到售后申请列表页
...
<style lang="scss" scoped>
    ...
    .completed /deep/ .ivu-modal-header {
