 ...
 * @Description: 重写DefaultTokenService 解决使用数据库存储配置信息时出现的莫名异常问题
 * @Date: 2019/8/9 21:55
 * @Version: V1.0
 */
@Component("singleTokenServices")
public class SingleTokenServices implements AuthorizationServerTokenServices, ResourceServerTokenServices, ConsumerTokenServices, InitializingBean {
    ...
    @PostConstruct
    private void init() {
        this.tokenStore = new MyRedisTokenStore(redisConnectionFactory);
    ...
    @Transactional(
            noRollbackFor = {InvalidTokenException.class, InvalidGrantException.class}, rollbackFor = Exception.class
    )
    @Override
...
//        }
    ...
    private OAuth2AccessToken createAccessToken(OAuth2Authentication authentication, OAuth2RefreshToken refreshToken) {
        ...
        // 完全由JWTConverter生成token与refresh后的token
        return jwtAccessTokenConverter.enhance(token, authentication);
//        return (OAuth2AccessToken)(this.accessTokenEnhancer == null ? jwtAccessTokenConverter.enhance(token, authentication) : token);
    }
