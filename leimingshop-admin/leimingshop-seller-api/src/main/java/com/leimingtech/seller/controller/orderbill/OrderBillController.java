...
package com.leimingtech.seller.controller.orderbill;
...
public class OrderBillController {
    ...
    public Result<PageData<BillTotalDTO>> page(@ApiIgnore @RequestParam Map<String, Object> params, @ApiIgnore SellerDetail sellerDetail) {
        if (sellerDetail == null || sellerDetail.getStoreId() == null) {
            return new Result<PageData<BillTotalDTO>>().error(ErrorCode.UNAUTHORIZED, "用户未登录");
        }
    ...
    @PutMapping
    @ApiOperation("商家备注")
    public Result update(@RequestBody StoreConfirmBillTotalDTO dto, @ApiIgnore SellerDetail sellerDetail) {

        if (sellerDetail == null || sellerDetail.getAccount() == null) {
            return new Result<>().error(ErrorCode.UNAUTHORIZED, "用户未登录");
        }

        billTotalService.updateStatus(dto, sellerDetail.getAccount());
