...
import com.leimingtech.seller.excel.aftersale.AftersaleReturnExcel;
...
public class AftersaleReturnController {
    ...
    public Result<PageData<AftersaleReturnPageDTO>> page(@ApiIgnore @RequestParam Map<String, Object> params, @ApiIgnore SellerDetail sellerDetail) {
        ...
        PageData<AftersaleReturnPageDTO> page = aftersaleReturnService.pageData(params, sellerDetail.getStoreId());
    ...
    @ApiOperation("订单退货详情")
    public Result<AftersaleReturnDetailDTO> detail(@PathVariable("aftersaleSn") Long aftersaleSn, @ApiIgnore SellerDetail sellerDetail) {
        if (null == sellerDetail || null == sellerDetail.getId()) {
            return new Result<AftersaleReturnDetailDTO>().error(ErrorCode.UNAUTHORIZED, "用户未登录");
        }
     ...
     */
    @PostMapping("confirm")
    @ApiOperation("确认收到退货商品")
    @LogOperation("确认收到退货商品")
    public Map<String, Object> confirm(@RequestParam("aftersaleSn") Long aftersaleSn, @RequestParam("aftersaleStatus") Integer aftersaleStatus, @ApiIgnore SellerDetail sellerDetail) {
        Map<String, Object> result = new HashMap<>(16);
        if (null == sellerDetail || null == sellerDetail.getId()) {
            result.put("code", ErrorCode.UNAUTHORIZED);
            result.put("msg", "用户未登录");
            return result;
        }
        result = aftersaleReturnService.confirmGoods(aftersaleSn, aftersaleStatus);
        return result;
    }
