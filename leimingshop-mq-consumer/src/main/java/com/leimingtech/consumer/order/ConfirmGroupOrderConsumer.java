 ...
 * @return
 * @date 2020-03-24 10:20
 **/
@Slf4j
@Component
public class ConfirmGroupOrderConsumer {

    private static MonitorLogger mlogger = MonitorLoggerFactory.getMonitorLogger(ConfirmGroupOrderConsumer.class);
     ...
     **/
    @RabbitHandler
    @RabbitListener(queues = MqConstant.LEIMINGTECH_CONFIRM_GROUP_ORDER_QUEUE)
    public void confirmOrder(Channel channel, Message message) throws IOException {
        ...
        log.info("接收到拼团订单异步保存消息，内容为:{}", orderConfirmId);
        try {
            // 异步处理订单保存
            orderService.packGroupOrderFromConsumer(Long.valueOf(orderConfirmId));
            ...
            saveOrderRedisDTO.setResultMsg("拼团订单保存失败");
            ...
            log.error("拼团订单生成失败" + e);
        }
        // ACK手动确认处理消息
        channel.basicAck(message.getMessageProperties().getDeliveryTag(), false);
        log.info("拼团保存订单处理完毕，手动执行ACK!");
    }
}
