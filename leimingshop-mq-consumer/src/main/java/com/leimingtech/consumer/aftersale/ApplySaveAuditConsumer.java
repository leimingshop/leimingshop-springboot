...
/**
 * @Author: huangkeyuan
 * @Description:售后申请阶段的保存日志消息
 * @Date: 2019/6/19 15:58
 * @Version: V1.0
 */
@Slf4j
@Component
public class ApplySaveAuditConsumer {
    ...
    @RabbitHandler
    @RabbitListener(queues = MqConstant.LEIMINGTECH_APPLY_SAVE_AUDIT_QUEUE)
    public void resetPasswd(Channel channel, Message message) throws IOException {
        String msgText = new String(message.getBody());
        log.info("接收到售后申请阶段保存日志消息，内容为：{}", msgText);
        ...
        AftersaleAuditLogSaveDTO aftersaleAuditLogSaveDTO = JSON.toJavaObject(jsonObject, AftersaleAuditLogSaveDTO.class);
        aftersaleAuditLogService.save(aftersaleAuditLogSaveDTO);

        // ACK手动确认处理消息
        channel.basicAck(message.getMessageProperties().getDeliveryTag(), false);

        log.info("售后申请阶段保存日志消息消息处理完毕，手动执行ACK!");
    }
}
