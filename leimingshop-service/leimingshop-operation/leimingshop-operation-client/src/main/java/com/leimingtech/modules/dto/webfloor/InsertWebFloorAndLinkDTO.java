 ...
 */
@Data
@ApiModel(description = "InsertWebFloorAndLinkDTO")
public class InsertWebFloorAndLinkDTO implements Serializable {
    private static final long serialVersionUID = 1L;

    @ApiModelProperty(value = "楼层名称")
    @NotBlank(message = "楼层名称不能为空", groups = AddGroup.class)
    @Size(max = 20, message = "楼层名称长度不大于20", groups = AddGroup.class)
    ...
    @Size(max = 100, message = "楼层名称图标长度不大于100", groups = AddGroup.class)
    private String nameIcon;

    @ApiModelProperty(value = "是否显示 0否 1是")
    @Min(value = 0, message = "请输入正确的显示类型", groups = AddGroup.class)
    @Max(value = 1, message = "请输入正确的显示类型", groups = AddGroup.class)
    private Integer isShow;

    @ApiModelProperty(value = "排序")
    @Min(value = 0, message = "排序范围为0-255", groups = AddGroup.class)
    @Max(value = 255, message = "排序范围为0-255", groups = AddGroup.class)
